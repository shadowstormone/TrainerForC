...
void BaseRender::CreateBaseWindow(LPCWSTR title, int width, int height)
{
	...
	wcx.hInstance = hInstance;										// Дескриптор экземпл¤ра, который содержит оконную процедуру для класса.
	...
	wcx.lpszClassName = L"Trainer By ShadowStorm";					// Символьна¤ строка, она задает им¤ класса окна
